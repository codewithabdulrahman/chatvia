...
<template>
  <ul class="list-unstyled mb-0" v-if="message.content !== '' || message.file !== null">
    <li :class="{ right: message.align === 'right' }">
      <div class="conversation-list" ref="imageRef" @mouseover="onHoverMessage(message)" @mouseleave="onLeaveMessage"
        :class="{
          'message-highlight': isMessageHover(message),
        }">
        <div class="user-chat-content">
          <div class="ctext-wrap">
            <div class="ctext-wrap-content message-box">
              <div v-if="
                roomUsers.length > 2 && message.sender_id !== currentUserId
              " class="text-username" :class="{
  'username-reply': !message.deleted && message.replyMessage,
}">
                <span>yu{{ message.username }}</span>
              </div>

              <div class="reply-message bg-white" v-if="!message.deleted && message.replyMessage">
                <div class="reply-username">{{ replyUsername }}</div>

                <div class="image-reply-container" v-if="isImageReply">
                  <div class="message-image message-image-reply" :style="{
  ...
  'max-height': `${imageResponsive.maxHeight}px`,
                ...
                <format-message :content="message.content" :textFormatting="textFormatting"></format-message>
              </div>
            ...
            <div class="options-container" :class="{ 'options-image': isImage && !message.replyMessage }" :style="{
              width:
              ...
              <transition-group name="slide-left">
                <div key="1" class="blur-container" :class="{
                  'options-me': message.sender_id === currentUserId,
                }" v-if="isMessageActions || isMessageReactions"></div>
                {{ isMessageReactions }}
                <emoji-picker key="3" class="message-reactions" :style="{ right: isMessageActions ? '30px' : '5px' }"
                  v-if="isMessageReactions" v-click-outside="closeEmoji" :emojiOpened="emojiOpened" :emojiReaction="true"
                  :roomFooterRef="roomFooterRef" :positionRight="message.sender_id === currentUserId"
                  @addEmoji="sendMessageReaction" @openEmoji="openEmoji"></emoji-picker>
              </transition-group>
            </div>
          </div>
          <transition-group v-if="!message.deleted">
            <button v-for="(reaction, key) in message.reaction" :key="key + 0" class="button-reaction">
              {{ reaction.icon }}
            </button>
          </transition-group>
        </div>
      ...
      " v-if="filteredMessageActions.length">
        <div ref="menuOptions" v-if="optionsOpened" v-click-outside="closeOptions" class="menu-options"
          :class="{ 'menu-left': message.sender_id !== currentUserId }" :style="{ top: `${menuOptionsTop}px` }">
          <div class="menu-list">
